...
if ! command -v node >/dev/null 2>&1; then
  echo "node is not installed. Install Node.js (e.g. 'sudo apt install nodejs npm') and rerun." >&2
  exit 1
fi

if ! command -v npm >/dev/null 2>&1; then
  echo "npm is not installed. Install npm (e.g. 'sudo apt install npm') and rerun." >&2
  exit 1
fi
...
if [[ ! -d "$WS_DIR" ]]; then
  echo "WS_DIR does not exist: $WS_DIR" >&2
  exit 1
fi
